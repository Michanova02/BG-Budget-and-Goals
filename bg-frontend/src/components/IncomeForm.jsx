...
function IncomeForm({ onIncomeAdded }) {
  ...
  // Detectar si el método de pago seleccionado es Transferencia
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        // Si no es transferencia, enviamos null para asegurar que no procese cuenta
      ...
      console.error(err);
      setMessage('Error al guardar el ingreso.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ backgroundColor: '#1a1d21', padding: '30px', borderRadius: '16px', border: '1px solid #2d3139' }}>
      <h3 style={{ color: '#fff', margin: '0 0 25px 0', fontSize: '20px', fontWeight: '500' }}>Registrar Ingreso</h3>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '20px' }}>
          <div style={{ flex: 2 }}>
            ...
            <input type="text" required value={descripcion} onChange={(e) => setDescripcion(e.target.value)} placeholder="Ej. Salario, Venta, Freelance..." style={inputStyle} />
          </div>
          <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          ...
          {/* El selector de cuentas solo se renderiza si el método elegido es Transferencia */}
          {esTransferencia && (
            <div style={{ flex: 1, minWidth: '180px' }}>
        ...
        <button type="submit" disabled={loading} style={{ backgroundColor: '#5fe3c0', color: '#131517', padding: '12px', border: 'none', borderRadius: '8px', fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', marginTop: '10px' }}>
          {loading ? 'Procesando...' : 'Añadir Ingreso'}
        </button>
